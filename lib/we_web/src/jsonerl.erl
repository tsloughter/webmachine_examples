...
-record(test_record, {foo, fu}).

update_record_test() ->
    R = ?json_to_record(test_record, #test_record{foo = <<"not bar">>, fu = <<"fubar">>}, "{\"foo\":\"bar\"}"),
    ?assertEqual(<<"bar">>, R#test_record.foo),
    ?assertEqual(<<"fubar">>, R#test_record.fu).
